...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        if let tabController = window?.rootViewController as? UITabBarController {
            tabController.viewControllers?.insert(packagedSearchController(), atIndex: 1)
    ...
    func packagedSearchController() -> UIViewController {
        ...
        guard let searchResultsController = storyboard.instantiateViewControllerWithIdentifier(SearchController.storyboardIdentifier) as? SearchController else {
